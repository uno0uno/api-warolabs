-- Script corregido para poblar inventario histórico sin duplicación
...
SELECT '============ POBLANDO INVENTARIO ============' as status;

-- Procesar compras con lógica corregida
...
cumulative_stock AS (
  SELECT
    ...
    ingredient_name,
    row_num,
    -- Stock acumulado por ingrediente
...
-- Insertar en tenant_inventory (solo el stock final por ingrediente)
...
SELECT DISTINCT ON (ingredient_id)
  ...
  ingredient_id,
  LAST_VALUE(new_stock) OVER (
    PARTITION BY ingredient_id
    ORDER BY row_num
    ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING
  ) as final_stock,
  0,
  MAX(purchase_date) OVER (PARTITION BY ingredient_id) as last_purchase_date
FROM cumulative_stock
ORDER BY ingredient_id, row_num DESC;

-- Insertar movimientos con stock correcto
WITH purchases_ordered AS (
  SELECT
    tp.id as purchase_id,
    tp.purchase_number,
...
SELECT '============ VERIFICACIÓN (Carne de Res) ============' as status;
...
WHERE ti.tenant_id = '0ffc1252-0bdf-467b-83e0-916213f9f1ec'
  AND i.name IN ('Carne de Res', 'Papa Ripio', 'Papas Fritas')
ORDER BY i.name;
